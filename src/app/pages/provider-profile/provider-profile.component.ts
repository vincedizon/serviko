...
export class ProviderProfileComponent implements OnInit {
  ...
  // The fixed Payment-First redirect
  book(): void {
  const selected = this.listingService.selectedListing();
  if (selected) {
  ...
  buildProvider(listing: Listing): void {
    this.provider = {
      ...
      about: (listing as any).description ?? `${listing.name} is a verified professional.`,
  ...
  loadReviews(listingId: string): void {
    this.http.get<{ success: boolean; data: any[] }>(
      ...
      next: (res) => {
        ...
        if (this.provider && this.reviews.length > 0) {
          const sum = this.reviews.reduce((acc, r) => acc + r.rating, 0);
          this.provider.rating = (sum / this.reviews.length).toFixed(1);
